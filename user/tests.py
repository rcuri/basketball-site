...
class ProfileViewTests(TestCase):
    """Testing Profile views using fixtures for initial data."""
    ...
    def test_sign_up_process(self):
        ...
        includes form validation and redirecting user to another page after
    ...
    def test_five_teams_assigned_signup(self):
        """
        Test whether only five teams are assigned to a user's team_set upon
        signing up for site.
        """
        client = Client()
        form_data = {
    ...
    def test_home_page_logged_in(self):
        """
        Test whether a logged in user can access the home page, which is only
    ...
    def test_home_page_logged_out(self):
        """
        Test whether a user is redirected upon attempting to access homepage,
        while not being signed into their account.
        """
        client = Client()
        form_data = {
            'username': 'homieman',
        ...
        [s.delete() for s in Session.objects.all() if (
            s.get_decoded().get('_auth_user_id')==str(testuser.id))]
