...
class TeamModelTests(TestCase):
    """Testing Team's models using fixtures for initial data."""
    ...
    def test_valid_total_teams(self):
        """
        Test whether the total number of teams for a season is the same as
        the total number of teams in our database. Change value of
...
class PlayerModelTests(TestCase):
    """Testing NBA Player's model using fixtures for initial data."""
    fixtures = ['basketball/fixtures/clean_database.json']

    def test_valid_team(self):
        """
        Test whether each player's assigned team exists in the team data set.
        """
...
class TeamsViewTest(TestCase):
    """Testing Team's view layer using fixtures for initial data."""
    fixtures = ['basketball/fixtures/profiles_loaded.json']

    def test_teams_page_accessible_logged_in(self):
        """
        Test to see if user is shown their teams page if they try to access
        it while logged into their account.
        """
        client = Client()
        user = User.objects.get(username='rodrigocuriel')
        self.client.force_login(user)
        response = self.client.get(reverse(
            'displayteams', kwargs= {'username':'rodrigocuriel'}))
        self.assertEqual(response.status_code, 200)

    def test_teams_page_inaccessible_logged_out(self):
        """
        Test to see if user is redirected to the 'index' page if they try to
        access another user's teams page.
        """
        client = Client()
        response = self.client.get(reverse(
            'displayteams', kwargs={'username':'rodrigocuriel'}))
